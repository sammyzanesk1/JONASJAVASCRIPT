...
//passin an arguement into event handler...always use d this keyword
...
//METHOD 1..USIN THE SCROLLEVENT APPROACH
// const initialCoords = section1.getBoundingClientRect(); //we set the VIEWPORT that has to be scrolled bfor the nav section bcoms sticky.
// console.log(initialCoords);
// window.addEventListener('scroll', function () {
//   console.log(window.scrollY);

//   if (window.scrollY > initialCoords.top) nav.classList.add('sticky');
//   //initialCoords.top(y axis height originally)...d nav bcoms sticky once we scroll beyond d original y axis height, i.e enters d first section.
//   else nav.classList.remove('sticky');
// });

//METHOD 2 USING THE INTERSECT API...

// const obsCallback = funtion(entries, obsevrer){
...
// obsevrer.observe(section1);

//step 1:create d element to be targeted..if we move out of dis element section, sticky nav is added to d page
const header = document.querySelector('.header'); //d header section will b our target. te sickey class will be added to d page immediately we completely go out of dis portion of d viewport.

//step 2: create d call back function for d obsever variable..i.e what we want to happen when d oberver detects dat we move out of d targeted area.
const stickyNav = function (entries) {
  const [entry] = entries; //we save d scroll nodelist to a variable..we will use d intersectin property of d nodelist...we will implement d sticky nav class based on d intersectin property of d nodelist.
  console.log(entry);

  if (!entry.isIntersecting) nav.classList.add('sticky');
  //we access d intersectin property of d variable to add or remove d sticky css class....if we move out of d header/target area add sticky class...note: !entry.isIntersecting=we have moved out of d targeted area, i.e d header scetion is no longer intersecting the viewport.
  else nav.classList.remove('sticky'); //if we are in d target area remove sticky class
};

//step 3: we create d observer variable which observes our target area...ofr d variable to do dis we use d new IntersectionObserver method..it has 2 arguement, d Intersection observer call back function, and Intersection observer options(root, rootMargin and threshold)
const headerObserver = new IntersectionObserver(stickyNav, {
  root: null, //we want to watch for intersection relative to the device's viewport.
  threshold: 0, //The degree of intersection between the target element(header) and its root(viewport) is the intersection ratio.we said zero ie.intersection happens wen d header section is completely out of d viewport(we scroll down and out of d header section completely)..note: wen d header is completely out of viewport intersectin property has a vaue of fasle. if d header is not completely out of d viewport intersectin is true.. just come here n tweak btw 0-1 to c d behavior in d viewport...A threshold of 1.0 means that when 100% of the target is visible within the element specified by the root option, the callback is invoked.
  rootMargin: '-90px', //d sticky nav should be implemented once we have scrolled 90% out of d target area.
});

//step 4: call d oberver function wit d target as its arguement.
headerObserver.observe(header);

///////////////////////////////////
//revealin sections on scroll
//select d target(s)
const allSections = document.querySelectorAll('.section');

//write d call back function
const revealSection = function (entries, oberver) {
  const [entry] = entries;
  // console.log(entry);
  if (!entry.isIntersecting) return;
  // if (entry.isIntersecting)
  entry.target.classList.remove('section--hidden'); //d section intersectin d veiwport by 15% or more is revealed...i.e d css class is implemented on d intersectin section.
...
const sectionObserver = new IntersectionObserver(revealSection, {
  root: null,
  threshold: 0.15, //implement css class wen d taret element is 15% visible.
});
...
///////////////////////////
//lazy loadin images...site performance(slow cellfone, small data, slow browsin network)
//step1: select target(s)
const imgTargets = document.querySelectorAll('img[data-src]'); //select only html image wit d [class]
...
const loadImg = function (entries, observer) {
  ...
  entry.target.src = entry.target.dataset.src; //replace original target image with data-src img

  //removin d blurred class/blur of d image after js replaces d lazy img wit d data-src img..only after d image has been replaced..so we lsiten for d image to complete loading i.e load event then implement d remove blur function
...
//step 3 write d observer function
const imgObserver = new IntersectionObserver(loadImg, {
  root: null,
  threshold: 0,
  rootMargin: '200px', //d image should b changed 200px
});

//step 4: call d observer function.
...
const slider = function () {
  //first we select d elements we want to work with...i.e sliders we want to make d images appear side by side acroos d width of d screen.

  const slides = document.querySelectorAll('.slide');

  //makin d slider button work
  const btnLeft = document.querySelector('.slider__btn--left');
  const btnRight = document.querySelector('.slider__btn--right');
  const dotContainer = document.querySelector('.dots');
  /*..addin elements to an empty parent element
  const sam = document.querySelector('.sam');
  console.log(sam);
  const createSams = function () {
  ...
  let curSlide = 0;
  const maxSlide = slides.length; //nodelist of maxslides(d list elements selected are 4..nodelist is not zero based, always remember)
  ...
  //activatin d active dot image button
  ...
  // slider.style.transform = 'scale(0.4) translateX (-800px)'; //reduce d scale
  // slider.style.overflow = 'visible';

  //slider callback function
  const goToSlide = function (slide) {
    slides.forEach(
      //takes in d slides elements selected above
      (s, i) => (s.style.transform = `translateX(${100 * (i - slide)}%)`) //u set d position of each iterated slide image by giving X a percentage...first image 0%..i.e 100*[0]
      //0, 100, 200, 300%
    );
  };
  // goToSlide(0); //start from poition [0] initially wen d gotoslide function is called

  //next slide right...//-100, 0, 100, 200%

  //we set d event handler function
  const nextSlide = function () {
    if (curSlide === maxSlide - 1) {
      //slider returns to first wen we get to d last image
      curSlide = 0;
    } else {
      curSlide++; //slider goes to next image
    }
    goToSlide(curSlide); //implement d slider callback function
  ...
  init();
  //event handlers
  //d slider functionality works on we clickin d slider
  btnRight.addEventListener('click', nextSlide);
  btnLeft.addEventListener('click', nextSlide);

  //////////////////////////
  ///keyboard event...slidin through d images using d keyboard arrows
  document.addEventListener('keydown', function (e) {
    console.log(e);
  ...
  //makin d dots work..usin event delegation
  dotContainer.addEventListener('click', function (e) {
    if (e.target.classList.contains('dots__dot')) {
      console.log('DOTS');
      const slide1 = e.target.dataset.slide; //returns an object wit properties and values related to d clicked button
      // console.log(slide1);
      const { slide } = e.target.dataset; //returns an object wit d clicked position as a property and value. we destructure d object to obtain te position dat was clicked as a number.
      console.log(slide);

      goToSlide(slide); //we call d gotoslide function wit d destructured number...basically wen we clik a button we go to d image.
      activateDot(slide);
    }
  });
};
...
//1.domcontent loaded event...i.e d tml file as been fully parsed, downloaded, executed and converted in d dom tree in d js ie dom ready...note it does not wait for external resources n images  to load just d html file and js file..we can listen to d event and add an hamdler

document.addEventListener('DOMContentLoaded', function (e) {
  console.log('HTML parsed and DOM tree built!', e);
});

//load event...it is fired by d window wen d htmlas been parsed and all images and external resources like css as been loaded...ie it is an event dat gets fired wen d complete page has loaded..we can listen to d event and add an hamdler
window.addEventListener('load', function (e) {
  console.log('Page fully loaded', e);
});

//before unload event...it is fired just bfor a user leaves or closes a page.
// window.addEventListener('beforeunload', function (e) {
//   e.preventDefault();
//   console.log(e);
//   e.returnValue = ''; //asks d user to confirm leave/abort page action by a pop up generic message
// });
